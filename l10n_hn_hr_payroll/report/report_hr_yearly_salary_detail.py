...
class EmployeesYearlySalaryReport(models.AbstractModel):
    _name = 'report.l10n_hn_hr_payroll.report_hryearlysalary'
    _description = "Yearly Salary Report"
    ...
    def get_employee_detail_new(self, form, employees, months):
        ...
        for payslip_lines_per_employee in payslip_lines:
            # if not payslip_lines_per_employee:
            #     allow_list.extend([['', ''] + null_values])
            #     deduct_list.extend([['', ''] + null_values])
            for line in payslip_lines_per_employee:
                for line[0] in line:
                    if line[0][0][0] == "GROSS":
                        ...
                        net = line[0]
                    elif line[0][13] > 0.0 and line[0][0][0] != "NET":
            ...
            allow_list_all.append(allow_list)
            deduct_list_all.append(deduct_list)
            allow_list = []
            deduct_list = []

        return allow_list_all, deduct_list_all, total

    def cal_monthly_amt(self, form, emp_id, months):
        result = []
        salaries = {}
        struct_id = form.get('struct_id', [])
        employee_company_id = form.get('employee_company_id', [])
        employee_branch_ids = form.get('employee_branch_ids', [])
        employee_company_id = employee_company_id and employee_company_id[0] or False
        struct_id = struct_id and struct_id[0] or False

        add_where_cond = ''
        if employee_company_id and not employee_branch_ids:
            add_where_cond = f'''and emp.department_id = {employee_company_id}'''
        if employee_branch_ids:
            f'''and emp.department_id in {tuple(employee_branch_ids)}'''
        if struct_id:
            add_where_cond += f""" and (p.struct_id = {struct_id})"""

        self.env.cr.execute(f'''SELECT rc.code, pl.name, sum(pl.total), \
                to_char(p.date_to,'mm-yyyy') as to_date  FROM hr_payslip_line as pl \
                LEFT JOIN hr_salary_rule_category AS rc on (pl.category_id = rc.id) \
                LEFT JOIN hr_payslip as p on pl.slip_id = p.id \
                LEFT JOIN hr_employee as emp on emp.id = p.employee_id \
                WHERE p.employee_id = %s {add_where_cond} and p.struct_id = %s \
        ...
        for code in categories:
            if code in salaries:
                res = self.salary_list(code, salaries[code], months)
                result.append(res)
        return result

    def salary_list(self, code, salaries, months):
        cat_salary_all = []
        for category_name, amount in salaries.items():
            cat_salary = []
            total = 0.0
            cat_salary.append([code, category_name])
            for mnth in months:
                if mnth != 'None':
                    if len(mnth) != 7:
                        mnth = '0' + str(mnth)
