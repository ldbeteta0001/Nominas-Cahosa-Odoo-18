...
class ReportHrSalaryEmployeeBymonth(models.AbstractModel):
    _name = 'report.l10n_hn_hr_payroll.report_hrsalarybymonth'
    _description = "Salary by Month Report"
    ...
    def get_salary(self, form, emp_id, emp_salary, total_mnths, mnths):
        ...
        if employee_company_id and not employee_branch_ids:
            where += f""" and (emp.department_id = '{employee_company_id[0]}')"""

        if employee_branch_ids:
            where += f""" and (emp.department_id IN {tuple(employee_branch_ids)})"""

        if struct_id:
            where += f""" and (p.struct_id = {struct_id})"""

        self.env.cr.execute(f"""{_select} {_from} {where} group by r.name, p.date_to, emp.id""", (
            emp_id, category_id))
