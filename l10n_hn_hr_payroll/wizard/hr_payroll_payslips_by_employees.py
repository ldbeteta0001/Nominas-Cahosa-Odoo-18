...
class HrPayslipEmployees(models.TransientModel):
    ...
    resource_calendar_id = fields.Many2one(
        'resource.calendar', 'Horario Laboral')

    @api.depends('department_id', 'contract_type_id', 'structure_id', 'resource_calendar_id')
        ...
        for wizard in self:
            ...
            if wizard.structure_id:
                domain = expression.AND([
                    domain,
                    [('contract_id.structure_type_id', '=', self.structure_id.type_id.id)]
                ])
            ...
            if wizard.resource_calendar_id:
                domain = expression.AND([
                    domain,
    ...
    def compute_sheet(self):
        ...
        for contract in self._filter_contracts(contracts):
            values = dict(default_values, **{
                ...
                'overtime_hours_for_all': self.overtime_hours_for_all,
