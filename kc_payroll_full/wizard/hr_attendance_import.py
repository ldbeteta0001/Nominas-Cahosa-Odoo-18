...
class HrAttendanceImport(models.TransientModel):
    ...
    def _find_existing_partial_attendance(self, employee, work_date):
        """
        Busca asistencias parciales existentes para completar (solo con check_in, sin check_out)
        """
    ...
    def action_import(self):
        ...
        try:
            ...
            # 2) Formato de Excel actual:
            # Columna A = ID Empleado (biometric_user_id)
            # Columna B = Nombre (información adicional)
            # Columna C = Departamento (información adicional)
            # Columna D = Fecha (YYYY-MM-DD)
            # Columna E = Hora (HH:MM:SS)
            ...
            for idx, row in enumerate(sheet.iter_rows(values_only=True), start=1):
                ...
                row_count += 1
                biometric_id = row[0] if len(row) > 0 else None
                employee_name = row[1] if len(row) > 1 else None
                department = row[2] if len(row) > 2 else None
                fecha_str = row[3] if len(row) > 3 else None
                hora_str = row[4] if len(row) > 4 else None

                _logger.info("Fila %d: ID=%s, Nombre=%s, Fecha=%s, Hora=%s", idx, biometric_id, employee_name, fecha_str, hora_str)

                if not biometric_id or not fecha_str or not hora_str:
                    empty_rows += 1
                    _logger.warning("Fila %d omitida: ID=%s, Fecha=%s, Hora=%s", idx, biometric_id, fecha_str, hora_str)
                    continue

                # Convertir ID biométrico
                try:
                    if isinstance(biometric_id, (int, float)):
                        barcode = str(int(biometric_id))
                    else:
                        barcode = str(biometric_id).strip()
                except Exception as e:
                    _logger.error("Error convirtiendo ID biométrico en fila %d: %s", idx, str(e))
                    error_count += 1
                    error_details.append({
                        'tipo': 'Error convirtiendo ID biométrico',
                        'fila': idx,
                ...
                try:
                    if isinstance(fecha_str, datetime):
                        ...
                        fecha_str_clean = fecha_str.strip()
                        fecha = datetime.strptime(fecha_str_clean, '%Y-%m-%d').date()
                    else:
                        fecha_str_clean = str(fecha_str).strip()
                        fecha = datetime.strptime(fecha_str_clean, '%Y-%m-%d').date()
                except Exception as e:
                    _logger.error("Error parseando fecha en fila %d: %s - Error: %s", idx, fecha_str, str(e))
                    error_count += 1
                    error_details.append({
                ...
                try:
                    if isinstance(hora_str, datetime):
                        hora = hora_str.time()
                    elif hasattr(hora_str, 'time'):
                        hora = hora_str.time()
                    elif isinstance(hora_str, str):
                        hora_str_clean = hora_str.strip()
                        try:
                            hora = datetime.strptime(hora_str_clean, '%H:%M:%S').time()
                        except:
                            try:
                                hora = datetime.strptime(hora_str_clean, '%H:%M').time()
                            except:
                                if '.' in hora_str_clean:
                                    decimal_hour = float(hora_str_clean)
                                    hours = int(decimal_hour * 24)
                                    minutes = int((decimal_hour * 24 - hours) * 60)
                                    seconds = int(((decimal_hour * 24 - hours) * 60 - minutes) * 60)
                                    hora = datetime.strptime(f'{hours:02d}:{minutes:02d}:{seconds:02d}', '%H:%M:%S').time()
                                else:
                                    raise ValueError(f'Formato de hora no reconocido: {hora_str_clean}')
                    else:
                    ...
                    _logger.error("Error parseando hora en fila %d: %s - Error: %s", idx, hora_str, str(e))
                    error_count += 1
                    error_details.append({
                        'tipo': 'Error parseando hora',
                        'fila': idx,
                        'empleado': barcode,
                        'hora': hora_str,
                        'error': str(e)
                ...
                # Las horas del Excel ya están en hora local, no se aplica ningún ajuste
                punch_datetime = datetime.combine(fecha, hora)
                
                _logger.info("Agregado: Barcode=%s, Tiempo=%s", barcode, punch_datetime)
                attend_list.setdefault(barcode, []).append(punch_datetime)
            ...
            for barcode, times in attend_list.items():
                try:
                    ...
                    for work_date, day_times in daily_marks.items():
                        ...
                        # PASO 5: NO aplicar +6. Las marcas ya están en hora local.
                        entries = classified_marks['entries']
                        exits = classified_marks['exits']

                        # PASO 6: Determinar qué crear/completar
                        if existing_partial:
                            # Completar asistencia parcial existente
                            if exits:
                                exit_time = exits[-1]  # Última salida (hora del Excel)
                                _logger.info(
                        ...
                        # PASO 6: NO aplicar +6. Las marcas ya están en hora local.
                        entries = classified_marks['entries']
                        exits = classified_marks['exits']
                        ...
                        if entries and exits:
                            ...
                            try:
                                ...
                                new_attendance = self.env["hr.attendance"].with_context(
                                    skip_extra_hours_detection=True
                            ...
                            entry_time = entries[0]  # Hora del Excel
                            _logger.info(
                            ...
                            exit_time = exits[0]  # Hora del Excel
                            _logger.info(
