...
class HrShiftRotation(models.Model):
    ...
    def action_search_employees_without_shift(self):
        ...
        # El turno está relacionado con el empleado pero se verifica a través del contrato activo
        ...
        # Analizar cada contrato para ver cuáles tienen empleados sin turno
        employees_without_shift_list = []
        employees_with_shift_list = []
        ...
        for contract in active_contracts:
            ...
            current_shifts = employee.shift_history_ids.filtered('is_current')
            has_current_shift = bool(current_shifts)
            total_history = len(employee.shift_history_ids)
            
            _logger.info("Empleado ID=%s, Nombre=%s: current_shift_period=%s, has_current_shift=%s, total_history=%s",
                        employee.id, employee.name, current_shift_period, has_current_shift, total_history)
            
            # Verificar si no tiene turno asignado
            # Un empleado tiene turno si tiene current_shift_period asignado (incluso si no tiene is_current=True)
            # porque el compute del empleado ya busca el último registro si no hay turno actual
            no_shift = not current_shift_period
            
            if no_shift:
                employees_without_shift_list.append(employee)
                _logger.info("  -> SIN TURNO (será incluido)")
            else:
                employees_with_shift_list.append(employee)
                _logger.info("  -> CON TURNO (será excluido): %s", current_shift_period)
        
        # Obtener empleados con contratos activos que no tienen turno asignado
        # Solo verificar si current_shift_period está vacío, no importa si tiene is_current
        ...
        _logger.info("Total empleados CON turno encontrados: %s", len(employees_with_shift_list))
        if employees_without_shift:
            _logger.info("IDs de empleados SIN turno: %s", employees_without_shift.ids)
        if employees_with_shift_list:
            _logger.info("IDs de empleados CON turno (excluidos): %s", [e.id for e in employees_with_shift_list])
        
        # Validar que los dos listados coincidan con el filtered
        _logger.info("=== VALIDACIÓN ===")
        if len(employees_without_shift_list) != len(employees_without_shift):
            _logger.warning("DISCREPANCIA: Lista manual (%s) vs Filtered (%s)", 
                          len(employees_without_shift_list), len(employees_without_shift))
        
        domain = [('id', 'in', employees_without_shift.ids)]
