...
class HrLoan(models.Model):
    ...
    def _onchange_employee_id(self):
        if self.employee_id:
            self.schedule_pay = self.employee_id.contract_id.schedule_pay
        ...
        for loan in self:
            ...
            for i in range(1, loan.installment + 1):
                ...
                if loan.employee_id.contract_id:
                    if loan.schedule_pay == 'weekly':
                        date_start = date_start + relativedelta(weeks=1)
                    elif loan.schedule_pay == 'bi-weekly':
                        date_start = date_start + relativedelta(days=15)
                    else:
                        date_start = date_start + relativedelta(months=1)
                else:
                    raise ValidationError('Debe tener el contrato en proceso')
