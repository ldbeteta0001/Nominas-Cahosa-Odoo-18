...
class HrViaticSheet(models.Model):
    ...
    place_and_date = fields.Char(string='Place and date', compute='_get_place_and_date', readonly=True)

    @api.depends('place', 'date')
    def _get_place_and_date(self):
        for rec in self:
            if rec.date and rec.place:
                rec.place_and_date = "%s-%s" % (rec.place, rec.date)
            else:
                rec.place_and_date = ''

    place = fields.Char(string='Place')
    date = fields.Date(string='Date to travel')
    travel_to = fields.Char(string='Travel to')
    reason_description = fields.Char(string='Reason for the trip')

    date_ini = fields.Date(string='Initial date')
    date_end = fields.Date(string='End date')

    employees_ids = fields.Many2many('hr.employee', 'expense_employee_rel', 'expense_id', 'employee_id',
                                     string='Employees related to expense')

    name = fields.Char(string="Viatic Report Summary", required=True, tracking=True)
    expense_line_ids = fields.One2many(
        comodel_name='hr.expense', inverse_name='viatic_id',
    ...
    total_amount_viatic = fields.Monetary(
        string="Total viatic",
        currency_field='company_currency_id',
        compute='_compute_amount', store=True, readonly=True,
        tracking=True,
    ...
    untaxed_amount_viatic = fields.Monetary(
        string="Untaxed Amount",
        currency_field='company_currency_id',
        compute='_compute_amount', store=True, readonly=True,
    )
    ...
    total_tax_amount_viatic = fields.Monetary(
        string="Taxes",
        currency_field='company_currency_id',
        compute='_compute_amount', store=True, readonly=True,
    )
    ...
    account_move_ids = fields.One2many(
        ...
        comodel_name='account.move', inverse_name='expense_viatic_sheet_id', readonly=True,
    )
    ...
    def _compute_amount(self):
        for sheet in self:
            ...
            sheet.total_amount_viatic = sum(sheet.expense_line_ids.mapped('sub_total_viatic_amount'))
            sheet.total_tax_amount_viatic = sum(sheet.expense_line_ids.mapped('tax_amount'))
            sheet.untaxed_amount_viatic = sheet.total_amount_viatic - sheet.total_tax_amount_viatic
    ...
    def create(self, vals_list):
        ...
        sheets = super(HrViaticSheet, self.with_context(context)).create(vals_list)
    ...
    def _unlink_except_posted_or_paid(self):
        for expense in self:
            if expense.account_move_ids.filtered(lambda move: move.state not in ['draft']):
                raise ValidationError(_("You cannot delete a report with posted or paid journal entries."))
            expense.account_move_ids.unlink()
    ...
    def activity_update(self):
        reports_requiring_feedback = self.env['hr.hn.viatic']
        reports_activity_unlink = self.env['hr.hn.viatic']
    ...
    def action_sheet_move_create(self):
        # Validar si se puede crear movimientos contables
        self._check_can_create_move()

        # Crear una hoja de gastos
        sheet_expense_id = self.env['hr.expense.sheet'].create({
            'name': self.name,
            'accounting_date': datetime.now(),
            'employee_id': self.employee_id.id,
            'expense_line_ids': [(6, 0, self.expense_line_ids.ids)],
            'viatic_id': self.id,  # Asociar la hoja de gastos al viático
        })

        # Vincular la hoja de gastos al reporte actual
        self.write({'sheet_id': sheet_expense_id.id, 'state': 'post'})

        # Actualizar las líneas de gasto con la referencia al viático
        self.expense_line_ids.write({'viatic_id': self.id})
    ...
    def _do_create_moves(self):
        ...
        own_account_sheets = self.filtered(lambda sheet: sheet.payment_mode == 'viatic')
        ...
        # for move_id in moves:
            ...
            #     if line_id.account_id.id == self.company_id.account_viatic_id.id:
            #         line_id.credit, line_id.debit = line_id.debit, line_id.credit
            #     elif line_id.account_id.id == self.company_id.partner_id.property_account_payable_id.id:
            #         line_id.date_maturity = datetime.today()
            #         line_id.account_id = self.company_id.partner_id.property_account_payable_id.id
    ...
    def _prepare_move_vals(self):
        ...
        return {
            ...
            'expense_viatic_sheet_id': self.id,
        }
    ...
    def action_done_viatic_sheets(self):
        pass